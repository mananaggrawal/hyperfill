...
`company-knowledge/`. Each RFP / RFE / tender arrives as a separate task. For each one:
...
You assemble facts from `company-knowledge/`; you do not invent them. If a required fact
...
|---|---|
| `company-knowledge/` — facts, profile text, submission documents | `bids/<slug>/` — one folder per RFP |
| `assets/` — letterhead, signature/stamp | `examples/` — finished reference bids (optional) |
| `toolkit/` — Python to build/convert/merge documents | |
| `docs/` — these guides | |
...
- **Identity & numbers** (registration no., tax IDs, offices, signatory, financials, escalation matrix):
  `company-knowledge/master-data.md` (human) and `toolkit/bidder_profile.py` (machine). Keep in sync.
- **Narrative** (what the company does, team, plans, security/technical posture):
  `company-knowledge/profile/`.
- **Attachable documents** (registration, tax, audited financials, certifications, experience proofs):
  `company-knowledge/submission-documents/`.
- **Branding for generated docs**: `assets/` (letterhead + signature/stamp).
- **Product/solution proposals**, if the user keeps them elsewhere: see
  `company-knowledge/proposal-library.md` (a place to record a live external reference).
...
- **Signature & stamp:** documents requiring a signature use the image in `assets/signature-stamp/`,
  inserted by `docx_builder.sign_block()` with the signatory from `bidder_profile`.
- **Letterhead:** generated documents are built on the `.docx` in `assets/letterhead/`.
...
- **Single source of truth:** never re-type a fact that lives in `bidder_profile.py` / `master-data.md`.
